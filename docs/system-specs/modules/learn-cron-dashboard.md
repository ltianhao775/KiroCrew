...
### Real-Time Debug Log (`debug_log`)

An AGENT (message) cron carries an optional `debug_log` field (bool, default `false`, absent reads as `false`) that streams a clean, structured, real-time trace of the tick to a per-job file an operator can `tail -f`: `cron-history/<job_id>.live.jsonl`. A cron agent tick is a separate `kiro-cli acp --agent` subprocess whose stdin/stdout/stderr the gateway holds as PIPEs for the ACP JSON-RPC transport, so the child's own output never reaches `gateway.log` and there is no tailable per-tick file at the default log level; the cron-history `<id>.jsonl` run record is written only after the run finishes, and the KiroCrew session transcript is flushed on a 5s/turn cadence (and is empty for a `hide_in_chat` cron). `debug_log` fills the gap those leave — an in-flight, structured view — WITHOUT raising `kiro-cli`'s own log level (that path, `KIRO_LOG_LEVEL=debug`, was rejected: it produces ~40 MiB/min of undocumented full-verbosity output dominated by low-level library noise, not a clean agent trace, and `KIRO_LOG_LEVEL` accepts only single level names — `error|warn|info|debug|trace` — so it cannot be narrowed by module).

The mechanism is a per-event tee on the stream KiroCrew already drives. `stream_and_collect` (`llm_helpers.py`) gains an observe-only `on_event(LLMEvent)` callback, fired for EVERY event before its own dispatch and wrapped so a raising observer never fails the turn; callers that pass no `on_event` are unaffected. The two cron agent dispatch sites (single-agent and the `agent_sequence` loop, both in `slack/gateway.py`) pass `on_event=make_cron_live_tee(self.cron_svc.history, job.id)` when `debug_log` is set, threaded through `_cron_stream_with_posttoken_resume`'s `**stream_kwargs`. The tee (`make_cron_live_tee`) projects each event through `live_event_record` into a redacted JSON record — `tool_call` (title + `redact`-ed input, plus the `tool_input_redacted` provenance bit), `tool_result` (status/final + `tool_output_digest`/`tool_output_bytes`, NEVER the raw `tool_output`), `complete` (stop_reason/refusal) — and aggregates consecutive `text_chunk` events into ONE `text` record flushed at the next tool/complete boundary (so the trace reads as whole assistant messages, not one line per token). Every free-text field is `redact`-ed (exfil-URL + credential passes) before it can reach the file, and a tool result's body is reduced to its digest, so a credential-shaped payload cannot land in the trace.

Writes go through `CronHistoryStore.append_live_event` (and `begin_live_trace`, which truncates the file at each run start so a follower always sees the run in flight — D1: each run overwrites the previous). These mirror `append`/`_append_sync` exactly: off-loop via `asyncio.to_thread`, under the `.history.lock`, `enabled`-gated, `_degrade` on OSError — best-effort, so a trace-write failure NEVER reaches the scheduler. The tee itself only schedules a fire-and-forget `ensure_future(append_live_event(...))`, so no blocking I/O runs on the event loop. The live file is not indexed and not capped like the run records; it is a transient tail reclaimed with the rest of the cron-history dir. `debug_log` is a no-op for `script`/`command` crons (no kiro-cli tick, no ACP event stream). Threaded through `_build_job`/`add_job`/`add_job_async`/`_update_job_locked` and persisted in `crons.json` via the dataclass `asdict` (`_job_from_record` defaults it to `false`); accepted on the `cron_add`/`cron_update` MCP tools (`FieldSpec("debug_log", bool)` in both `CRON_ADD_SCHEMA`/`CRON_UPDATE_SCHEMA`), on `POST`/`PATCH /api/crons`, and surfaced in the `cron_list` drill-in and `GET /api/crons`. It is independent of `agent.log_level`, which sets the gateway's own `kiro_crew` logger level (what reaches `gateway.log` / `kirocrew logs -f`).
